...
export default function ReactionLabScreen({ onClose }: { onClose: () => void }) {
  ...
  return (
    <View style={S.root}>
      ...
      {(query || filter !== 'all' || favoritesOnly) && <TouchableOpacity style={S.favoriteFilter} accessibilityRole="button" accessibilityLabel="Clear reaction search and filters" onPress={() => { setQuery(''); setFilter('all'); setFavoritesOnly(false); setSpotlight(null); }}><Text style={S.favoriteFilterText}>Clear search & filters</Text></TouchableOpacity>}
